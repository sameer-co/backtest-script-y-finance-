...
# Compatibility Patch for Backtrader
...
class AdvancedRSIStrategy(bt.Strategy):
    ...
    def next(self):
        if not self.position:
            ...
            if rsi_cross and trend_strong and above_sma:
            ...
            # 1. Take Partial Profit at RR Target
            if not self.half_booked and self.data.high[0] >= self.target:
                self.sell(size=int(self.position.size / 2))
                self.half_booked = True
                self.stop_loss = max(self.stop_loss, self.data.open[0]) # Move to Breakeven
            
            # 2. Dynamic Trailing Stop (Post-Half Booking)
            if self.half_booked:
